...
func loginRouter(t *testing.T) (http.Handler, *auth.Service) {
	...
	r := NewRouter(cfg, seedFake(), runner.Runner(&fakeRunner{}), nil, nil, AuthConfig{Service: svc}, nil)
	return r, svc
}
...
func TestLogin_WithSlogLogger(t *testing.T) {
	...
	cfg := config.Config{Env: config.EnvDev, FileStore: config.FileStoreLocal, AuthLocalEnabled: true}
	r := NewRouter(cfg, seedFake(), runner.Runner(&fakeRunner{}), nil, nil, AuthConfig{Service: svc, Logger: logger}, nil)
...
func TestLogin_NotRegisteredWhenLocalAuthDisabled(t *testing.T) {
	...
	r := NewRouter(config.Config{Env: config.EnvDev, FileStore: config.FileStoreLocal}, seedFake(), nil, nil, nil, AuthConfig{}, nil)
