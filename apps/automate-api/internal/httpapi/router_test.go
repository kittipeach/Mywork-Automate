...
func TestRouter_Readyz(t *testing.T) {
	r := NewRouter(config.Config{Env: config.EnvSIT, FileStore: config.FileStoreLocal}, seedFake(), nil, nil, nil, AuthConfig{}, nil)
...
func TestRouter_AuthConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			r := NewRouter(tt.cfg, seedFake(), nil, nil, nil, AuthConfig{}, nil)
